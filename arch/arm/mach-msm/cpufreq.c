...
#if defined(CONFIG_SLP_MINI_TRACER)
	kernel_mini_tracer("CPUF_TARGET[S]\n", TIME_ON | FLUSH_CACHE);
#endif
...
#if defined(CONFIG_SLP_MINI_TRACER)
	kernel_mini_tracer("CPUF_TARGET[E]\n", TIME_ON | FLUSH_CACHE);
#endif
...
#ifdef CPUFREQ_MAX_1GHZ
#define CPUFREQ_TABLE_VALID_ENTRY		5
#endif
...
#ifdef CPUFREQ_MAX_1GHZ
	/* Configuring available cpu frequencies for RINATO 3G*/
	nf = CPUFREQ_TABLE_VALID_ENTRY;
#endif
...
static int pm_qos_min_cpu_notifier_call(struct notifier_block *this,
			    unsigned long cpufreq, void *ptr)
{
	int j=0;
	...
	for_each_present_cpu(j) {
		...
		if (policy !=NULL) {
			if (cpufreq > policy->cur)
				msm_cpufreq_driver.target(policy, cpufreq, CPUFREQ_RELATION_L);
...
static struct notifier_block pm_qos_min_cpu_notifier = {
	.notifier_call = pm_qos_min_cpu_notifier_call,
};
...
static int __init msm_cpufreq_register(void)
{
	...
	pm_qos_add_notifier(PM_QOS_CPU_FREQ_MIN, &pm_qos_min_cpu_notifier);
