				 ...
				 enum setrate_reason reason)
{
	...
	if (cpu_online(cpu) && cpu_active(cpu))
		strlcpy(cpu_state[cpu], "Online", ARRAY_SIZE(cpu_state[cpu]));
	else if (!cpu_online(cpu) && cpu_active(cpu))
		strlcpy(cpu_state[cpu], "Migrating",
			ARRAY_SIZE(cpu_state[cpu]));
	else if (!cpu_online(cpu) && !cpu_active(cpu))
		strlcpy(cpu_state[cpu], "Down", ARRAY_SIZE(cpu_state[cpu]));
	else
		strlcpy(cpu_state[cpu], "On/NotActive",\
			 ARRAY_SIZE(cpu_state[cpu]));
