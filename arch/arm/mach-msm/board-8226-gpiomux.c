...
#if defined(CONFIG_MACH_RINATO_3G_OPEN) || defined(CONFIG_MACH_B3)
...
static struct msm_gpiomux_config b2_3g_hw_check_bit_cfg[]={
	{
		.gpio = 13,
		.settings = {
...
static struct msm_gpiomux_config b2_3g_nc_cfg[]={
	{
		.gpio = 6,
		.settings = {
...
static struct msm_gpiomux_config msm_lcd_oled_det_config[] __initdata = {
	{
		.gpio = 62,		/* OLED_DET */
		.settings = {
...
static struct msm_gpiomux_config msm_lcd_esd_det_config[] __initdata = {
	{
		.gpio = 74,		/* ESD_DET */
		.settings = {
			[GPIOMUX_ACTIVE]    = &lcd_esd_act_cfg,
...
static struct msm_gpiomux_config msm_lcd_io_config[] __initdata = {
	{
		.gpio = 73,		/* LCD_IO_EN */
		.settings = {
			[GPIOMUX_ACTIVE]    = &nc_cfg,
...
#ifdef CONFIG_MACH_RINATO_3G_OPEN
static struct msm_gpiomux_config ssp_nc_cfg[]={
	{
		.gpio = 62,
		.settings = {
			[GPIOMUX_ACTIVE] = &nc_cfg,
...
void __init msm8226_init_gpiomux(void)
{
	...
	msm_gpiomux_install(msm_lcd_oled_det_config,
				ARRAY_SIZE(msm_lcd_oled_det_config));
#else
	msm_gpiomux_install_nowrite(msm_lcd_configs,
			ARRAY_SIZE(msm_lcd_configs));
	if (system_rev > 0x5)
		...
		msm_gpiomux_install(msm_lcd_esd_det_config,
				ARRAY_SIZE(msm_lcd_esd_det_config));

	if (system_rev < 0x5)
		msm_gpiomux_install_nowrite(msm_lcd_io_config,
				ARRAY_SIZE(msm_lcd_io_config));
#endif
...
#ifdef CONFIG_MACH_RINATO_3G_OPEN
	if (system_rev < 5) {
		msm_gpiomux_install(ssp_nc_cfg, ARRAY_SIZE(ssp_nc_cfg));
		pr_info("[SSP] %s, system_rev= %u, Set GPIO as NC\n", __func__, system_rev);
	}
#endif
#endif
	msm_gpiomux_install(b2_3g_nc_cfg, ARRAY_SIZE(b2_3g_nc_cfg));
	msm_gpiomux_install(b2_3g_hw_check_bit_cfg, ARRAY_SIZE(b2_3g_hw_check_bit_cfg));
#endif
