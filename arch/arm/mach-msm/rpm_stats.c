...
static struct platform_device *rpm_stats_dev;
...
#ifdef CONFIG_PM_SLEEP_HISTORY
static int msm_rpmstats_get_stats(
	void __iomem *reg,
	int record,
	struct msm_rpmstats_record *precord)
{
	precord->val = msm_rpmstats_read_register(reg, record, 2);

	return 0;
}

static inline int msm_rpmstats_get_stats_v2(
			void __iomem *reg,
			int record,
			struct msm_rpm_stats_data_v2 *pdata)
{
	pdata->stat_type = msm_rpmstats_read_long_register_v2(reg, record,
			offsetof(struct msm_rpm_stats_data_v2,
				stat_type));
	pdata->count = msm_rpmstats_read_long_register_v2(reg, record,
			offsetof(struct msm_rpm_stats_data_v2, count));
	pdata->last_entered_at = msm_rpmstats_read_quad_register_v2(reg,
			record, offsetof(struct msm_rpm_stats_data_v2,
				last_entered_at));
	pdata->last_exited_at = msm_rpmstats_read_quad_register_v2(reg,
			record, offsetof(struct msm_rpm_stats_data_v2,
				last_exited_at));

	pdata->accumulated = msm_rpmstats_read_quad_register_v2(reg,
			record, offsetof(struct msm_rpm_stats_data_v2,
				accumulated));
	return 0;
}
...
{
	struct rpm_stats *rpm_stats = platform_get_drvdata(rpm_stats_dev);
	struct msm_rpmstats_platform_data *pdata = rpm_stats->pdata;
	void __iomem *reg_base;
	int num_records;
	...
	if (pdata->version == 1) {
		...
		num_records = readl_relaxed(reg_base);
		msm_rpmstats_get_stats(reg_base, 1, &record);
		ptime->v1.version = pdata->version;
		ptime->v1.sleep = record.val;
	} else if (pdata->version == 2) {
		int i;
		struct msm_rpm_stats_data_v2 data[2];

		num_records = 2;
		for (i=0; i < num_records; i++)
			msm_rpmstats_get_stats_v2(reg_base, i, &data[i]);\

		ptime->v2.version = pdata->version;
		ptime->v2.xosd = (u32)get_time_in_sec(data[0].accumulated);
		ptime->v2.vmin = (u32)get_time_in_sec(data[1].accumulated);
	}
...
EXPORT_SYMBOL(mms_rpmstats_get_total_sleep_time);
#endif

static  int __devinit msm_rpmstats_probe(struct platform_device *pdev)
{
#ifdef CONFIG_PM_SLEEP_HISTORY
	struct rpm_stats *rpm_stats;
	struct msm_rpmstats_platform_data *pdata;
	...
	rpm_stats = kzalloc(sizeof(struct rpm_stats), GFP_KERNEL);

	if (!rpm_stats) {
		kfree(pdata);
		return -ENOMEM;
	}
	rpm_stats->pdata = pdata;
	...
	if (!ret) {
		...
		rpm_stats->dent = debugfs_create_file("rpm_stats", S_IRUGO, NULL,
	...
	platform_set_drvdata(pdev, rpm_stats);
	rpm_stats_dev = pdev;
	return 0;
...
{
#ifdef CONFIG_PM_SLEEP_HISTORY
	struct rpm_stats *rpm_stats = platform_get_drvdata(pdev);

	debugfs_remove(rpm_stats->dent);
