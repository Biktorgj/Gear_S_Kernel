...
static int msm_buspm_bus_set_ex(struct msm_buspm_bus_set bs)
{
	unsigned int type = 0x0;
	unsigned int id = 0x0;
	unsigned int key = 0x0;
	...
	pr_info("bs.op=%d type=0x%x key=0x%x id=%x bs.set=%d bs.min=%d bs.max=%d\n"
		,bs.op, type, key, id, bs.set,bs.min, bs.max);
...
int msm_set_pmqos_bimc_min_freq(void)
{
	...
	bs.nocid = 0x10;
	bs.set = 0;
	bs.max =  pm_qos_request(PM_QOS_BIMC_FREQ_MAX);
	bs.min =  pm_qos_request(PM_QOS_BIMC_FREQ_MIN);

	bs.op = MSM_BUSPM_BUS_MIN_SET;
	ret = msm_buspm_bus_set_ex(bs);
