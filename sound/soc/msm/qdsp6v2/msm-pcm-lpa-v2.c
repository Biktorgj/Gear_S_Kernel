...
#define MUSIC_CPUFREQ_VALUE 787200
#endif
static int msm_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (!pm_qos_request_active(&cpufreq_min_qos)) {
		...
		PM_QOS_CPU_FREQ_MIN, MUSIC_CPUFREQ_VALUE);
	} else {
		pm_qos_update_request(&cpufreq_min_qos, MUSIC_CPUFREQ_VALUE);
	}
#endif
	return 0;
}
