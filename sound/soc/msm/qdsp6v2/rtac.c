...
{
	u32 i = 0;
	pr_debug("%s: port_id = %d, popp_id = %d num v2 dev = %d \n", __func__, port_id,
		popp_id, rtac_adm_data_v2.num_of_dev );
	...
	if (rtac_adm_data_v2.num_of_dev != 0) {
		for (; i < rtac_adm_data_v2.num_of_dev; i++) {
			if (rtac_adm_data_v2.device[i].afe_port == port_id && rtac_adm_data_v2.device[i].copp == copp_id) {
...
static void rtac_remove_adm_device_v2(u32 port_id, u32 copp_id) 
{
	s32 i;
	pr_debug("%s: port_id = %d copp_id = %d num v2  dev =%d\n", __func__, port_id, copp_id, rtac_adm_data_v2.num_of_dev );
...
{
	u32 i = 0;
	pr_debug("%s: port_id = %d, popp_id = %d num dev =%d \n", __func__, port_id,
		popp_id, rtac_adm_data.num_of_dev );
...
{
	s32 i;
	pr_debug("%s: port_id = %d num  dev =%d \n", __func__, port_id, rtac_adm_data.num_of_dev);

	mutex_lock(&rtac_adm_mutex);
	rtac_remove_adm_device_v2(port_id, copp_id);
