...
static int es705_uart_boot_setup(struct es705_priv *es705)
{
	...
	dev_info(es705->dev, "%s(): write ES705_SBL_SYNC_CMD = 0x%02x\n",
	...
	dev_info(es705->dev, "%s(): sbl sync ack = 0x%02x\n", __func__, rspn);
	...
	/* SBL BOOT BYTE 0x01 */
	dev_info(es705->dev, "%s(): write ES705_SBL_BOOT_CMD = 0x%02x\n",
	...
	dev_info(es705->dev, "%s(): sbl boot ack = 0x%02x\n", __func__, rspn);
es705_bootup_failed:
	return rc;
}
...
#endif
	dev_info(es705->dev, "%s(): write ES705_SYNC_CMD = 0x%08x\n",
		__func__, sync_cmd);
es705_boot_retry:
	...
	dev_info(es705->dev, "%s(): firmware load success 0x%08x\n",
		__func__, sync_resp);
