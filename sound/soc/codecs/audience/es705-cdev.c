...
static int stream_read_error;
#define MAX_READ_ERROR 15
...
static int streaming_open(struct inode *inode, struct file *filp)
{
	...
	es705 = container_of((inode)->i_cdev, struct es705_priv,
			     cdev_streaming);
	dev_dbg(es705->dev, "%s(), f_count %ld\n", __func__, file_count(filp));
			      ...
			      size_t count, loff_t *f_pos)
{
	...
	if (stream_read_error >= MAX_READ_ERROR) {
		dev_err(es705->dev, "%s() exceed max uart read error.\n",
					__func__);
		err = -EIO;
		goto ERR_OUT;
	}
...
read_next_page:
		...
		while (!stream_read_page) {
			err = wait_event_interruptible_timeout(es705->stream_in_q,
				(stream_read_page =
				streaming_consume_page(&length)), HZ/2);
			...
			if (err == -ERESTARTSYS) {
				...
				dev_err(es705->dev, "%s(): timeout happend\n",
							__func__);
				err = -EIO;
				goto ERR_OUT;
			}
	...
	dev_dbg(es705->dev, "%s() bytes_read read  = %ld, f_count %ld\n",
					__func__, bytes_read, file_count(filp));
	return bytes_read;

ERR_OUT:
	stream_read_error = 0; /* reset error count */
	dev_err(es705->dev, "%s() exit by error %d\n", __func__, err);
	return err;
}
	...
	do {
		...
		if (data_ready > 0) {
			rlen_last = es705->streamdev.read(es705, buf + rlen,
				PAGE_SIZE - rlen);
			if (rlen_last < 0) {
				if (MAX_READ_ERROR < ++stream_read_error)
					stream_read_error = MAX_READ_ERROR;
				dev_err(es705->dev, "%s(): read error on streamdev: %d, err %d\n",
					__func__, rlen_last, stream_read_error);
			} else {
				rlen += rlen_last;
