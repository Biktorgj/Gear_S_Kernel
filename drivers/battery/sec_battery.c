				...
				struct sec_battery_info *battery)
{
	...
	*/
	#if 0
	battery->capacity = 60;
	battery->voltage_now = 3200;
	battery->current_now = 1360;
	battery->temperature = 20;
	battery->current_max = 20000;
	#endif
...
static void sec_booting_check_work(struct work_struct *work)
{
	...
	pr_info("%s booting check\n", __func__);

	/* booting 0, completed 1 */
	batt_booting_chk = 1;
...
//#if 0
	...
	schedule_delayed_work(&battery->batt_booting_check_work, msecs_to_jiffies(20000));
