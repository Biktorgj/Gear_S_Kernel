...
#if defined(CONFIG_USB_G_SLP) && defined(CONFIG_USB_CI13XXX_MSM)
	/* MSM udc used already spin_lock_irqsave for queueing
	 * So does not need spin_unlock/lock for queueing
	 */
...
int acm_notify(void *dev, u16 state)
{
	...
	if (dev) {
		...
		if (acm->notify->driver_data) {
			...
			/* MSM udc worked irq-handler based (not threaded) so
