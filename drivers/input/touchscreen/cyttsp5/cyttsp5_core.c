		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (command_code != hid_output->command_code) {
		...
		dev_err(cd->dev,
			"%s: 222HID output response, wrong command_code:%X\n",
...
static void cyttsp5_startup_work_function(struct work_struct *work)
{
	...
	complete(&cd->startup_work_complete);
...
int cyttsp5_core_start(struct device *dev)
{
	...
	if (cd->report_rate != DEFAULT_REPORT_RATE) {
		...
		cd->report_rate = DEFAULT_REPORT_RATE;
	}
...
int cyttsp5_core_ambient_on(struct device *dev)
{
	...
	if ((cd->silicon_id) == QFN_SILICON_ID || (cd->silicon_id) == BGA_SILICON_ID) {
		cyttsp5_core_sleep(cd);
	} else {
...
int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (!cd->device_enabled) {
		dev_info(dev, "%s: already device disabled.\n", __func__);
		return 0;
	}
...
int cyttsp5_core_resume(struct device *dev)
{
	...
	if (cd->sleep_wake_error) {
		dev_info(dev, "%s: sleep wakeup error happend.\n", 
			__func__);
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->lcd_id = get_lcd_id();
	if (!cd->lcd_id) {
		dev_err(dev, "%s: lcd id is zero.\n", __func__);
		rc = -ENODEV;
		goto error_alloc_data;
	}
	...
	init_completion(&cd->startup_work_complete);
