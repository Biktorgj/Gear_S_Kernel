...
#define CALIBRATION_RETRY_TEST 0
...
recal:
	...
	if (rc < 0) {
		dev_err(dev, "%s: fail request exclusive\n", __func__);
		goto exit;
	}

	rc = cmd->cmd->suspend_scanning(dev, 0);
#if (CALIBRATION_RETRY_TEST == 1)
	rc = -1;
#endif
	if (rc < 0) {
		dev_err(dev, "%s: fail suspend scanning\n", __func__);
		goto release;
	}
	...
	for (mode = 0; mode < 3; mode++) {
		...
		if (rc < 0) {
			dev_err(dev, "%s: fail calibrate idacs\n", __func__);
			goto release;
		}
	}
	...
	if (rc < 0) {
		dev_err(dev, "%s: fail resume scanning\n", __func__);
		goto release;
	}
...
release:
	if (cmd->release_exclusive(dev) < 0)
		dev_err(dev, "%s: fail release exclusive\n", __func__);
		...
		int fw_size)
{
	...
	if (cd->probe_done) {
		...
		wait_for_completion(&cd->startup_work_complete);
	}
		...
		const struct firmware *fw)
{
	...
	if (cd->silicon_id == BGA_SILICON_ID) {
