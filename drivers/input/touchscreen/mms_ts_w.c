...
#include "mms_ts_w_firmware.h"
#include "mms_ts_w_config_fw.h"
...
#define RINATO_3G_MODEL_STR "RINATO-3G"
...
static int mms_config_get(struct mms_ts_info *info, u8 mode)
{
	...
	if (mode == REQ_FW) {
		...
		info->config_fw = kzalloc(sizeof(mms_ts_w_config_fw), GFP_KERNEL);
		if (!info->config_fw) {
			dev_err(&client->dev, "%s: Failed to allocate new requeset\n", __func__);
			return -ENOMEM;
		}
		memcpy((void *)info->config_fw, mms_ts_w_config_fw, sizeof(mms_ts_w_config_fw));
	} else if (mode == UMS) {
		old_fs = get_fs();
		set_fs(get_ds());
...
static int mms_ts_fw_load(struct mms_ts_info *info)
{
	...
	/* firmware update */
	do {
		ret = mms_flash_fw(mms_ts_w_firmware,
					info, COMPARE_UPDATE);
	} while (ret && --retries);
...
static void fw_update(void *device_data)
{
	...
	if(info->cmd_param[0] == REQ_FW) {
		...
		if (IS_ERR(fp)) {
			...
			fsize = fp->f_path.dentry->d_inode->i_size;
			if (fsize == 0) {
				dev_err(&client->dev, "%s: %s size is zero.\n",
					__func__, fw_path);
				filp_close(fp, current->files);
				goto not_support;
			}
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s_%s_0x%02x", RINATO_3G_MODEL_STR,
			info->config_fw_version, config_ver);
...
static int mms_ts_input_open(struct input_dev *dev)
{
	...
	if (!info->init_done) {
		dev_err(&info->client->dev,
			"%s: failed to open mms_ts. init_done:[%d]\n",
			__func__, info->init_done);
		return -1;
	}
...
static int melfas_power(int onoff)
{
	int rc;

	printk(KERN_INFO "[TSP] %s called with ON= %d\n",__func__, onoff);

	if(!vddo_vreg || !avdd_vreg){
		pr_err("[TSP]No regulator!!\n");
	}

	if(onoff){
		rc = regulator_enable(vddo_vreg);
		if(rc){
			pr_err("[TSP] unable to enable vddo\n");
			return rc;
		}
		rc = regulator_enable(avdd_vreg);
		if(rc){
			pr_err("[TSP] unable to enable avdd\n");
			return rc;
		}
	}else{
		rc = regulator_disable(vddo_vreg);
		if(rc){
			pr_err("[TSP] unable to disable vddo\n");
			return rc;
		}
		rc = regulator_disable(avdd_vreg);
		if(rc){
			pr_err("[TSP] unable to disable avdd\n");
			return rc;
		}
	}
	msleep(100);
...
static int melfas_init_gpio(struct mms_ts_info *info)
{
	int rc;
	...
	if (IS_ERR(vddo_vreg)){
		vddo_vreg = NULL;
		printk(KERN_INFO "[TSP] info->vddo_vreg error\n");
		return -1;
	}
	...
	if (IS_ERR(avdd_vreg)){
		avdd_vreg = NULL;
		printk(KERN_INFO "[TSP] info->avdd_vreg error\n");
		return -1;
	}
	rc = regulator_set_voltage(avdd_vreg, 3300000, 3300000);
	if(rc){
		pr_err("[TSP] unable to set voltage for avdd_vreg\n");
		return rc;
	}
	return 0;
}
                        ...
                        struct melfas_tsi_platform_data *pdata)
{
	...
	of_property_read_string(np, "melfas,config_fw_version", &pdata->config_fw_version);
	return 0;
}
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node){
		...
		ret = melfas_init_gpio(info);
		if (ret) {
			dev_err(&client->dev, "Error inting reg,gpio%d\n", ret);
			return ret;
		}

		info->pdata->power = melfas_power;
