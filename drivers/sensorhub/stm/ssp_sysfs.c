...
if (iRet < 0)
	goto err_create_uv_poll_delay;
#endif

	return SUCCESS;

#ifdef CONFIG_SENSORS_SSP_UVIS25
err_create_uv_poll_delay:
	pr_err("[SSP] %s could not create uv poll delay node\n", __func__);
#endif
#ifdef CONFIG_SENSORS_SSP_ADPD142
...
err_create_hrm_poll_delay:
	...
	device_remove_file(&data->game_rot_input_dev->dev,
		&dev_attr_input_game_rot_poll_delay);
err_create_game_rot_input_dev:
	pr_err("[SSP] %s could not create game rot poll delay node\n", __func__);
#endif
	device_remove_file(&data->rot_input_dev->dev,
		&dev_attr_input_rot_poll_delay);
err_create_rot_input_dev:
	pr_err("[SSP] %s could not create rot poll delay node\n", __func__);
#ifdef CONFIG_SENSORS_SSP_AL3320
	device_remove_file(&data->light_input_dev->dev,
		&dev_attr_input_light_poll_delay);
err_create_light_input_dev:
	pr_err("[SSP] %s could not create light poll delay node\n", __func__);
#endif
#ifdef CONFIG_SENSORS_SSP_LPS25H
	device_remove_file(&data->pressure_input_dev->dev,
		&dev_attr_input_pressure_poll_delay);
err_create_pressure_poll_delay:
	pr_err("[SSP] %s could not create pressure poll delay node\n",
		__func__);
#endif
	device_remove_file(&data->uncalib_gyro_input_dev->dev,
		&dev_attr_input_uncalib_gyro_poll_delay);
err_uncalib_gyro_poll_delay:
	pr_err("[SSP] %s could not create uncali gyro poll delay node\n",
		__func__);
	device_remove_file(&data->uncal_mag_input_dev->dev,
		&dev_attr_input_uncal_mag_poll_delay);
err_create_uncal_mag_poll_delay:
	pr_err("[SSP] %s could not create uncal mag poll delay node\n", __func__);
	device_remove_file(&data->mag_input_dev->dev,
		&dev_attr_input_mag_poll_delay);
err_create_mag_poll_delay:
	pr_err("[SSP] %s could not create mag poll delay node\n", __func__);
