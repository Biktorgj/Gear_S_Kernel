...
#define DUMP_FILE_PATH "/opt/usr/media/Downloads/MCU_DUMP"
...
{
	int iLength;
	u8 iLength_low = pchRcvDataFrame[(*pDataIdx)++];
	u8 iLength_high = pchRcvDataFrame[(*pDataIdx)++];
	int cur = *pDataIdx;

	iLength = ((iLength_high << 8) | iLength_low);
