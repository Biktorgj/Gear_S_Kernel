...
#define KENEL_TRACER_QUEUE_SIZE (1024)
...
unsigned int cpu_load_history_num = 102;
...
struct cpu_load_freq_history_tag {
	...
	unsigned int power_domains[2];
	unsigned int clk_gates[5];
#endif
...
#if defined(CONFIG_SLP_KERNEL_ENG)
#define SAVED_TASK_INFO_NUM 300
#else
#define SAVED_TASK_INFO_NUM 30
#endif

#define SAVED_TASK_INFO_SIZE (sizeof(struct saved_task_info_tag)\
											*SAVED_TASK_INFO_NUM)
...
static unsigned long long calc_delta_time_work(\
...
static unsigned int view_list(int max_list_num, \
...
static unsigned int view_list_raw(char *buf,\
...
#if defined (CONFIG_CHECK_WORK_HISTORY)
static unsigned int view_workfn_list(char *buf,\
									unsigned int buf_size, unsigned int ret)
{
	struct list_head *p;
	struct cpu_work_runtime_tag *curr;
			...
			, unsigned int start_cnt, unsigned int end_cnt)
{
	...
	for (i = start_array_num, cnt = 0; cnt < loop_cnt; cnt++, i++) {
		...
		work_runtime_data = search_exist_workfn(\
...
void store_cpu_load(unsigned int cpufreq[], unsigned int cpu_load[])
{
	...
	unsigned long long t;
	unsigned long  nanosec_rem;
	int cpu_max_freq_lock = 0, cpu_min_freq_lock = 0;
	...
	if (policy !=NULL) {
		cpu_min_freq_lock = pm_qos_request(PM_QOS_CPU_FREQ_MIN);
		if (cpu_min_freq_lock < policy->min)
			cpu_min_freq_lock = policy->min;

		cpu_max_freq_lock = pm_qos_request(PM_QOS_CPU_FREQ_MAX);
		if (cpu_max_freq_lock > policy->max)
			cpu_max_freq_lock = policy->max;

		cpufreq_cpu_put(policy);
	}

	cpu_load_freq_history[cnt].cpu_min_locked_freq = cpu_min_freq_lock;
	cpu_load_freq_history[cnt].cpu_max_locked_freq = cpu_max_freq_lock;
	...
	if ((value_for_debug == 1) || (value_for_debug == 3)) {
		...
		cpu_load_freq_history[cnt].cnoc_clk = ( get_cnoc_clk() + 50000) / 100000;
		cpu_load_freq_history[cnt].pnoc_clk = (get_pnoc_clk() + 50000) / 100000;
		cpu_load_freq_history[cnt].snoc_clk = (get_snoc_clk()+ 50000) / 100000;
	} else {
		cpu_load_freq_history[cnt].bimc_clk = CONFIG_DEFAULT_VAL;
		cpu_load_freq_history[cnt].cnoc_clk = CONFIG_DEFAULT_VAL;
		cpu_load_freq_history[cnt].pnoc_clk = CONFIG_DEFAULT_VAL;
		cpu_load_freq_history[cnt].snoc_clk = CONFIG_DEFAULT_VAL;
	}
...
unsigned int show_cpu_load_freq_sub(int cnt, int show_cnt, \
											...
											char *buf, unsigned int buf_size, int ret)
{
	...
	for (j = 0; j < show_cnt; j++) {
		...
		"    [%5d]\t%3d\t%3d\t%3d " \
		"\t%2d.%1d     %3d.%1d    %3d.%1d    %3d.%1d     %3d.%1d       %3d     %3d.%1d\
			\n"
		, cpu_load_freq_history_view[cnt].time
		, cpu_load_freq_history_view[cnt].cpufreq[0]/1000000
		, (cpu_load_freq_history_view[cnt].cpufreq[0]/10000) % 100
...
void cpu_last_load_freq(unsigned int range, int max_list_num)
{
	...
	ret +=  snprintf(buf + ret, BUF_SIZE - ret
		, "    TIME       CPU0   CPU1 CPU_LOCK"
		"    [INDEX]\tCPU0 \tCPU0  tONLINE\tNR_RUN\n");
	...
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		...
		ret +=  snprintf(buf + ret, PAGE_SIZE - ret
			, "    TIME   STA  CPU0_F  CPUF_LOCK  ON_LOCK"
			"  [INDEX]\tCPU0 \tCPU1" \
				  " \tONLINE\tNR_RUN\n");
	}
	...
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		...
		ret +=  snprintf(buf + ret, PAGE_SIZE - ret
			, "TIME       C0_F  C1_F  C2_F  C3_F   CPU_LOCK"
			"  [INDEX]  CPU0  CPU1" \
			" CPU2 CPU3 ONL NR_RUN" \
			"  BIMC   CNOC   PNOC  SNOC  GPUFQ GPU_UT"
			"    PD1       PD2      CLK1    CLK2        CLK3       CLK4\n");
	}
...
extern int fixed_cpu_num; /*  definition location
									: qc-linux/arch/arm/mach-msm/msm_rq_stats.c */
static int hotplug_lock_read_sub(char *buf, int buf_size)
{
	int ret = 0;
...
static ssize_t hotplug_lock_read(struct file *file,
	char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy;

	size_for_copy = wrapper_for_debug_fs(buffer, count, ppos,\
														hotplug_lock_read_sub);
...
static ssize_t __ref hotplug_lock_write(struct file *file,
				      const char __user *user_buf, size_t count,
				      loff_t *ppos)
{
	int ret = 0, input = 0;

	ret = sscanf(user_buf, "%d", &input);
	if (ret < 0)
		return -EINVAL;
...
extern int min_cpu_num_lock; /*  definition location :
									qc-linux/arch/arm/mach-msm/msm_rq_stats.c */
extern int max_cpu_num_lock; /*  definition location :
									qc-linux/arch/arm/mach-msm/msm_rq_stats.c */

static int min_cpu_num_lock_read_sub(char *buf, int buf_size)
{
	int ret = 0;
...
static ssize_t min_cpu_num_lock_read(struct file *file,
	char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy;

	size_for_copy = wrapper_for_debug_fs(buffer, count, ppos,\
												min_cpu_num_lock_read_sub);

	return size_for_copy;
}

static ssize_t min_cpu_num_lock_write(struct file *file,
				      const char __user *user_buf, size_t count,
				      loff_t *ppos)
{
	int ret = 0, input = 0;
	...
	if (((1 <=input) && (input <= num_possible_cpus())) || (input == -1))
		if ((input <= max_cpu_num_lock) || (max_cpu_num_lock == -1) || (input == -1))
			min_cpu_num_lock = input;
		else
			return -EINVAL;
	else
		return -EINVAL;
...
static int max_cpu_num_lock_read_sub(char *buf, int buf_size)
{
	int ret = 0;
...
static ssize_t max_cpu_num_lock_read(struct file *file,
	char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy;

	size_for_copy = wrapper_for_debug_fs(buffer, count, ppos,\
												max_cpu_num_lock_read_sub);

	return size_for_copy;
}

static ssize_t max_cpu_num_lock_write(struct file *file,
				      const char __user *user_buf, size_t count,
				      loff_t *ppos)
{
	int ret = 0, input = 0;
	...
	if (((1 <=input) && (input <= num_possible_cpus())) || (input == -1))
		if ((input >= min_cpu_num_lock) || (min_cpu_num_lock==-1) || (input == -1))
			max_cpu_num_lock = input;
		else
			return -EINVAL;
	else
		return -EINVAL;
...
int saved_range_num=50, saved_max_list_num=5;
static int last_cpu_load_read_sub(char *buf, int buf_size)
{
	...
	cnt = cpu_load_freq_history_view_cnt - 1;
	ret = show_cpu_load_freq_sub(cnt, saved_range_num, buf, (BUF_SIZE - ret) ,ret);
	ret +=  snprintf(buf + ret, BUF_SIZE - ret, "\n");

	end_cnt = cnt;
	start_cnt = get_index(end_cnt, cpu_load_history_num, (0 -saved_range_num + 1) );

	sprintf(range_buf, "%d-%d", start_cnt ,end_cnt);
	set_cpu_load_freq_history_array_range(range_buf);

	ret = check_running_buf_print(saved_max_list_num, buf, (BUF_SIZE - ret), ret);
...
static const struct file_operations hotplug_lock_fops = {
	.owner = THIS_MODULE,
	.read = hotplug_lock_read,
	.write =hotplug_lock_write,
};

static const struct file_operations min_cpu_num_lock_fops = {
	.owner = THIS_MODULE,
	.read = min_cpu_num_lock_read,
	.write =min_cpu_num_lock_write,
};

static const struct file_operations max_cpu_num_lock_fops = {
	.owner = THIS_MODULE,
	.read = max_cpu_num_lock_read,
	.write =max_cpu_num_lock_write,
};
...
if (!debugfs_create_file("saved_load_analyzer_data_num", 0600
		...
		if (!debugfs_create_file("hotplug_lock", 0600
			, d, NULL,&hotplug_lock_fops))   \
				pr_err("%s : debugfs_create_file, error\n", "hotplug_lock");

		if (!debugfs_create_file("min_cpu_num_lock", 0600
			, d, NULL,&min_cpu_num_lock_fops))   \
				pr_err("%s : debugfs_create_file, error\n", "min_cpu_num");

		if (!debugfs_create_file("max_cpu_num_lock", 0600
			, d, NULL,&max_cpu_num_lock_fops))   \
				pr_err("%s : debugfs_create_file, error\n", "max_cpu_num");
		...
		if (!debugfs_create_file("available_cpu_freq", 0600
			, d, NULL,&available_cpu_freq_fops))   \
				pr_err("%s : debugfs_create_file, error\n", "available_cpu_freq");

		if (!debugfs_create_file("last_cpu_load", 0600
			, d, NULL,&last_cpu_load_fops))   \
				pr_err("%s : debugfs_create_file, error\n", "last_cpu_load");
#if defined(CONFIG_SLP_MINI_TRACER)
		if (!debugfs_create_file("mini_tracer_view", 0600
