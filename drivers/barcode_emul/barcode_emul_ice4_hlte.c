...
struct regulator *barcode_l19_3p3=NULL;
...
static int ice4_clock_en(int onoff)
{
...
#else	
	if (!fpga_main_src_clk){
		fpga_main_src_clk = clk_get(NULL, "cam_gp1_src_clk");
	}
	if (IS_ERR(fpga_main_src_clk)) {
   		pr_err( "%s: unable to get fpga_main_src_clk\n", __func__);
   	}
	...
	if (!fpga_main_clk){
		fpga_main_clk = clk_get(NULL, "cam_gp1_clk");
	}
	if (IS_ERR(fpga_main_clk)) {
   		pr_err( "%s: unable to get fpga_main_clk\n", __func__);
   	}
...
static void barcode_gpio_config(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_FPGA_MAIN_CLK, \
		3, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);	
	...
	|| defined(CONFIG_MACH_HLTEDCM) || defined(CONFIG_MACH_HLTEKDI) \
	|| defined(CONFIG_MACH_JS01LTEDCM) || defined(CONFIG_MACH_JS01LTESBM) \
	|| defined(CONFIG_SEC_LOCALE_KOR_FRESCO) || defined(CONFIG_MACH_RINATO_3G_OPEN)
		...
		const struct i2c_device_id *id)
{
	...
	&& !defined(CONFIG_MACH_RINATO_3G_OPEN)
