			...
			struct dss_module_power *mp, struct device_node *node)
{
	...
	for_each_child_of_node(of_node, supply_node) {
		if (!strncmp(supply_node->name, "qcom,platform-supply-entry",
			...
			if (rc) {
				printk("%s: error reading supply pre sleep value. rc=%d\n",
					__func__, rc);
			}
			...
			if (rc) {
				printk("%s: error reading supply post sleep value. rc=%d\n",
					__func__, rc);
			}
			...
			printk("%s: %s min=%d, max=%d, enable=%d, disable=%d, preonsleep=%d, postonsleep=%d, preoffsleep=%d, postoffsleep=%d\n",
				__func__,
				...
				int enable, int partial)
{
	...
	if (!partial && !__mdss_dsi_ulps_feature_enabled(pdata)) {
		printk("%s: ULPS feature not supported. enable=%d\n",
			__func__, enable);
		return -ENOTSUPP;
	}
	...
	printk("%s: DSI lane status = 0x%08x. Ulps %s\n", __func__,
	...
	int enable)
{
	...
	if (mctrl) {
		printk("%s: configuring ulps (%s) for master ctrl%d\n",
	...
	printk("%s: configuring ulps (%s) for ctrl%d\n",
		__func__, (enable ? "on" : "off"), ctrl->ndx);
...
static int mdss_dsi_dfps_config(struct mdss_panel_data *pdata, int new_fps)
{
	...
	if (new_fps !=
		...
		printk("%s: Panel is already at this FPS\n", __func__);
	}
		...
		struct platform_device *pdev)
{
	...
	printk("%s:%d: Select primary panel from dt\n",
					__func__, __LINE__);
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (1) {
		...
		printk("%s:%d: no cmd line cfg present\n",
			 __func__, __LINE__);
		goto end;
	} else {
		if (ctrl_id == 1)
...
static int __devinit mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	printk("%s: Dsi Ctrl->%d initialized\n", __func__, index);
	return 0;
			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	rc = msm_dss_ioremap_byname(pdev, &ctrl->mmss_misc_io,
		"mmss_misc_phys");
	if (rc) {
		printk("%s:%d mmss_misc IO remap failed\n",
			__func__, __LINE__);
	}
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (dynamic_fps) {
		...
		if (data) {
			if (!strcmp(data, "dfps_suspend_resume_mode")) {
				...
				printk("%s: dfps mode: suspend/resume\n",
								__func__);
			} else if (!strcmp(data,
					    "dfps_immediate_clk_mode")) {
				pinfo->dfps_update =
						DFPS_IMMEDIATE_CLK_UPDATE_MODE;
				printk("%s: dfps mode: Immediate clk\n",
								__func__);
			} else if (!strcmp(data,
					    "dfps_immediate_porch_mode")) {
				pinfo->dfps_update =
					DFPS_IMMEDIATE_PORCH_UPDATE_MODE;
				printk("%s: dfps mode: Immediate porch\n",
								__func__);
			} else {
				printk("%s: dfps to default mode\n",
								__func__);
				pinfo->dfps_update =
						DFPS_SUSPEND_RESUME_MODE;
				printk("%s: dfps mode: suspend/resume\n",
			...
			printk("%s: dfps update mode not configured\n",
								__func__);
				pinfo->dynamic_fps =
								false;
				printk("%s: dynamic FPS disabled\n",
								__func__);
		}
	...
	printk("%s: Panel data initialized\n", __func__);
