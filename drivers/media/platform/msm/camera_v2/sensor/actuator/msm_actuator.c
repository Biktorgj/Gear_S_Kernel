...
static struct msm_actuator msm_piezo_sb280s_actuator_table;
...
static struct msm_actuator *actuators[] = {
	...
	&msm_piezo_sb280s_actuator_table,
};
...
static int32_t msm_actuator_init_sb280s_focus(struct msm_actuator_ctrl_t *a_ctrl,
	uint16_t size, enum msm_actuator_data_type type,
	struct reg_settings_t *settings)
{
	int32_t rc = -EFAULT;
#if defined (MSM_ACUTUATOR_DEBUG)
	int32_t i = 0;
	uint8_t data[100];
	...
	/* 2. Write Reg. 0x1D = 0x00 supplied from userspace. */
	rc = msm_actuator_init_focus(a_ctrl, size, type, settings);
	if (rc < 0) {
		pr_err("%s:%d Failed rc = %d\n", __func__, __LINE__, rc);
		return rc;
	}

	/* 3. Read Infinity data. */
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
	   &a_ctrl->i2c_client, 0x14 | 0x40, &a_ctrl->sb280s.infinity_data,
	   MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0) {
		pr_err("%s:%d Failed I2C read rc:%d\n", __func__, __LINE__, rc);
		return rc;
	}

	/* 4. Read Macro data */
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
	   &a_ctrl->i2c_client, 0x16 | 0x40, &a_ctrl->sb280s.macro_data,
	   MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0) {
		pr_err("%s:%d Failed I2C read rc:%d\n", __func__, __LINE__, rc);
		return rc;
	...
	/* 4. Read ADC Min Data */
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
	   &a_ctrl->i2c_client, 0x18 | 0x40, &a_ctrl->sb280s.adc_min_data,
	   MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0) {
		pr_err("%s:%d Failed I2C read rc:%d\n", __func__, __LINE__,rc);
		return rc;
	...
	/* 5. Read ADC Max Data */
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
	   &a_ctrl->i2c_client, 0x1A | 0x40, &a_ctrl->sb280s.adc_max_data,
	   MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0) {
		pr_err("%s:%d Failed I2C read rc:%d\n", __func__, __LINE__,rc);
		return rc;
	...
	CDBG("Inf_data = 0x%X\n",a_ctrl->sb280s.infinity_data);
	CDBG("Macro_data = 0x%X\n",a_ctrl->sb280s.macro_data);
	CDBG("Adc_min_data = 0x%X\n",a_ctrl->sb280s.adc_min_data);
	CDBG("Adc_max_data = 0x%X\n",a_ctrl->sb280s.adc_max_data);

#if defined(MSM_ACUTUATOR_DEBUG)
	/* Memory data check. */
	num_bytes = 7;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x9 | 0x40, &data[i], num_bytes);
	i += num_bytes;

	num_bytes = 2;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x12 | 0x40, &data[i], num_bytes);
	i += num_bytes;

	num_bytes = 1;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x1C | 0x40, &data[i], num_bytes);
	i += num_bytes;

	num_bytes = 1;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x1E | 0x40, &data[i], num_bytes);
	i += num_bytes;

	num_bytes = 1;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x20 | 0x40, &data[i], num_bytes);
	i += num_bytes;

	num_bytes = 16;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x22 | 0x40, &data[i], num_bytes);
	i += num_bytes;

	num_bytes = 4;
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client, 0x33 | 0x40, &data[i], num_bytes);
	i += num_bytes;
...
static int32_t msm_actuator_sb280s_set_position(
	struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_set_position_t *set_pos)
{
	int32_t rc = 0;
	...
	a_ctrl->i2c_tbl_index = 0;
	for (index = 0; index < set_pos->number_of_steps; index++) {
		CDBG("set_pos->pos[%d] = 0x%x\n",index, set_pos->pos[index]);

		next_lens_pos = set_pos->pos[index] << 4;
		CDBG("next_lens_pos = 0x%x\n",next_lens_pos);

		a_ctrl->i2c_reg_tbl[a_ctrl->i2c_tbl_index].reg_addr =
			(next_lens_pos & 0xFF00) >> 8;
	...
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_write_table_w_microdelay(
			&a_ctrl->i2c_client, &reg_setting);
	if (rc < 0) {
		pr_err("%s:%d Failed I2C write rc:%d\n", __func__, __LINE__, rc);
		return rc;
	}
	...
	void __user *argp)
{
	...
	case CFG_GET_ACTUATOR_INFO:
		...
		cdata->calib_info.infinity_data = a_ctrl->sb280s.infinity_data;
		cdata->calib_info.macro_data = a_ctrl->sb280s.macro_data;
		cdata->calib_info.adc_min_data = a_ctrl->sb280s.adc_min_data;
		cdata->calib_info.adc_max_data = a_ctrl->sb280s.adc_max_data;
#endif
		break;
...
static struct msm_actuator msm_piezo_sb280s_actuator_table = {
	.act_type = ACTUATOR_PIEZO_SB280S,
	.func_tbl = {
		.actuator_init_step_table = NULL,
		.actuator_move_focus = NULL,
		.actuator_write_focus = NULL,
		.actuator_set_default_focus = NULL,
		.actuator_init_focus = msm_actuator_init_sb280s_focus,
		.actuator_parse_i2c_params = msm_actuator_parse_i2c_params,
		.actuator_set_position = msm_actuator_sb280s_set_position,
		.actuator_sw_landing = NULL,
	},
};
