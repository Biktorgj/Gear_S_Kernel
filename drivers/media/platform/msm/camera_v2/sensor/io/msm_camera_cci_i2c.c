	...
	struct msm_camera_i2c_reg_setting *write_setting)
{
	...
	if (write_setting->data_type == MSM_CAMERA_I2C_BURST_DATA) {
...
#if defined(CONFIG_MACH_RINATO_3G_OPEN)
	        reg_array = (struct msm_camera_i2c_reg_array *)(write_setting->reg_setting);
#endif
		cci_ctrl.cmd = MSM_CCI_I2C_WRITE;
		cci_ctrl.cci_info = client->cci_client;
		...
		cci_ctrl.cfg.cci_i2c_write_cfg.size = write_setting->size;
			...
			CDBG("%s:%d reg[%d] = 0x%x data[%d] %x delay = 0x%x\n", __func__, __LINE__, i,
				reg_array[i].reg_addr, i, reg_array[i].reg_data, reg_array[i].delay);
		}
#endif
		rc = v4l2_subdev_call(client->cci_client->cci_subdev,
				core, ioctl, VIDIOC_MSM_CCI_CFG, &cci_ctrl);
